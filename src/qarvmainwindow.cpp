...
QArvMainWindow::QArvMainWindow(QWidget* parent, bool standalone_) :
    ...
    toDisableWhenPlaying(), toDisableWhenRecording(), futureHoldsAHistogram(
        false),
            ...
            "Some icons are not available in your theme, using bundled icons.";
    {
        ...
        int j = y % 19; int k = y / 100; int h = y % 100; int
            ...
            q = (k - p + 1) / 3; int r = (19 * j + k - m - q + 15
                                          ) % 30; int s = h / 4; int u = h % 4;
        int v = (32 + 2
                 *
                 n + 2 * s - r - u) % 7; int w = (j + 11 * r + 22
                                                  *
                                                  v) / 451;
        int x = r + v - 7 * w + 114; int z = x % 31
                                             +
                                             1;
        x = x / 31;
        if (l == x && (z == a || z+1 == a || z-1 == a)) {
            ...
            connect(r,
                    SIGNAL(clicked(bool)),
                    SLOT(on_replayButton_clicked(bool)));
...
void QArvMainWindow::on_refreshCamerasButton_clicked(bool clicked) {
    ...
    int previous_cam;
    if (data.isValid()
        && (previous_cam = cameraSelector->findText(data.toString())) != -1)
...
void QArvMainWindow::on_replayButton_clicked(bool checked) {
    const char k[] =
    { 72, 97, 112, 112, 121, 32, 69, 97, 115, 116, 101, 114, 33, 0 };
    const char u[] =
    { 71, 108, 97, 100, 32, 121, 111, 117, 32, 102, 111, 117, 110, 100, 32, 116,
      104, 105, 115, 33, 0 };
...
skip_all_file_opening:
    ...
    foreach (auto wgt, toDisableWhenRecording) {
        wgt->setEnabled(!recording && !open);
        on_videoFormatSelector_currentIndexChanged(
            videoFormatSelector->currentIndex());
    }
...
void QArvMainWindow::on_snapshotAction_toggled(bool checked) {
    ...
    if (snappathEdit->text().isEmpty() || snapbasenameEdit->text().isEmpty()) {
        tabWidget->setCurrentWidget(recordingTab);
        statusBar()->showMessage(tr(
                                     "Please set the snapshot directory and name."),
                                 statusTimeoutMsec);
...
void QArvMainWindow::updateBandwidthEstimation() {
    ...
    if (mtu == 0) {
        cameraMTUDescription->setText(
            tr("Not an ethernet camera or unsupported feature."));
    } else {
        ...
        QString description =
            tr("Camera is on interface %1,\npacket size set to %2.");
...
bool ToolTipToRichTextFilter::eventFilter(QObject* obj, QEvent* evt) {
    if (evt->type() == QEvent::ToolTipChange) {
        ...
        while (NULL != (parent = parent->parent())) {
            if (parent->metaObject()->className()
                == QString("QArv::QArvMainWindow")) {
                doEnrich = true;
                break;
            }
        }
...
void QArvMainWindow::saveProgramSettings() {
    ...
    for (auto i = saved_widgets.begin(); i != saved_widgets.end(); i++) {
        ...
        if (auto* w = qobject_cast<QCheckBox*>(widget))
            ...
            logMessage() << "FIXME: don't know what to save under setting"
                         << i.key();
    }
}
...
void QArvMainWindow::on_postprocRemoveButton_clicked(bool checked) {
    ...
    if (!item)
        return;
    auto editor = var2ptr<ImageFilterSettingsDialog>(item->data(
                                                         Qt::UserRole + 2));
    if (editor) {
        editor->close();
        editor->deleteLater();
