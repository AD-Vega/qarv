...
#include "decoders/yuyv422.h"
#include <QDebug>

YUYV422Decoder::YUYV422Decoder(QSize size_) :
  ...
  image_pointers { 0, 0, 0 }, image_strides { 0, 0, 0 } {
    ...
    ctx = sws_getContext(size.width(), size.height(), PIX_FMT_YUYV422,
                         size.width(), size.height(), PIX_FMT_RGB24,
                         SWS_BICUBIC, 0, 0, 0);
}

YUYV422Decoder::~YUYV422Decoder() {
  sws_freeContext(ctx);
}

QImage YUYV422Decoder::decode(QByteArray frame) {
...
Q_EXPORT_PLUGIN2(YUYV422, YUYV422Format)